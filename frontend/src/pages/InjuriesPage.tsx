      ...
      {isLoading && (
        <ul style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {Array.from({ length: 5 }).map((_, i) => (
            <li key={i} style={{ background: "#111827", borderRadius: "0.75rem", height: "64px" }} />
          ))}
        </ul>
      )}

      {data && data.length === 0 && (
        <div style={{ background: "#052e16", border: "1px solid #166534", borderRadius: "0.75rem", padding: "1.5rem", textAlign: "center" }}>
