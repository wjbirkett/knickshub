...
function opponentFromArticle(a) {
    if (a.article_type === "history")
        return null;
    if (a.home_team?.includes("Knicks") || a.home_team?.includes("New York"))
        return a.away_team;
    return a.home_team;
}
function slugToOpponent(slug) {
...
export default function MatchupArchivePage() {
    ...
    const matchupArticles = (allArticles ?? []).filter((a) => {
    ...
    if (isLoading)
        return <p style={{ color: "#6b7280", padding: "1rem" }}>Loading matchup...</p>;
    if (!allArticles)
        return <p style={{ color: "#f87171", padding: "1rem" }}>Could not load articles.</p>;
    return (<div style={{ maxWidth: "900px", margin: "0 auto", padding: "1rem" }}>
      ...
      {sortedDates.length === 0 && (<div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: "0.75rem", padding: "2rem", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        {sortedDates.map(gameDate => {
            ...
            return (<div key={gameDate} style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: "0.75rem", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                {articles.map((a) => {
                    ...
                    return (<Link key={a.slug} to={`/predictions/${a.slug}`} style={{ textDecoration: "none" }}>
                      <div style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", padding: "0.875rem 1.25rem", borderBottom: "1px solid #1f2937", transition: "background 0.15s" }} onMouseEnter={e => (e.currentTarget.style.background = "#0d1117")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
