...
export default function MatchupArchivePage() {
    ...
    const oppName = opponent?.split("-").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ") ?? "";
    const matchupArticles = articles?.filter((a) => {
    ...
    return (<div style={{ background: S.bg, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "2rem 2.5rem", maxWidth: "1000px" }}>
        {matchupArticles.length === 0 ? (<p style={{ color: S.textMuted }}>No articles found for this matchup yet.</p>) : (<div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {matchupArticles.map((a) => {
                const badge = TYPE_CONFIG[a.article_type] ?? TYPE_CONFIG.prediction;
                const result = predictions.find((r) => r.game_date === a.game_date);
                return (<div key={a.slug} style={{ background: S.surface, padding: "1.25rem", display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    <Link to={`/predictions/${a.slug}`} style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 700, fontSize: "0.9375rem", textTransform: "uppercase", color: S.text, textDecoration: "none" }} onMouseEnter={e => (e.currentTarget.style.color = S.peach)} onMouseLeave={e => (e.currentTarget.style.color = S.text)}>{a.title.replace(/\s*\([\d-]+\)\s*$/, "")}</Link>
