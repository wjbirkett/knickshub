...
export default function BettingRecordPage() {
  ...
  const mlHits   = preds.filter((r: any) => r.moneyline_result === "HIT").length
  const mlTotal  = preds.filter((r: any) => r.moneyline_result).length
  const propHits = props.filter((r: any) => r.result === "HIT").length

  const pct = (h: number, t: number) => t > 0 ? Math.round(h/t*100) : 0
  const color = (h: number, t: number) => pct(h,t) >= 55 ? S.green : pct(h,t) >= 45 ? S.peach : S.red
      ...
      <div style={{ padding: "2rem 2.5rem", maxWidth: "1200px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "1rem", marginBottom: "2.5rem" }}>
          {[
            ...
            <div key={label as string} style={{ background: S.surface, padding: "1.5rem", borderTop: `3px solid ${color(h as number, t as number)}` }}>
              ...
              <p style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 900, fontSize: "2rem", color: color(h as number, t as number), margin: "0 0 0.25rem" }}>{h as number}-{(t as number)-(h as number)}</p>
              <p style={{ fontSize: "0.75rem", color: S.textMuted, margin: 0 }}>{pct(h as number, t as number)}% · {t as number} picks</p>
            </div>
          ))}
        </div>
          ...
          <section style={{ marginBottom: "2.5rem" }}>
            ...
            <div style={{ background: S.surface, overflow: "hidden" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto auto auto", gap: 0, borderBottom: `1px solid ${S.border}`, padding: "0.625rem 1rem" }}>
                {["Game", "Spread", "Total", "ML", "Date"].map(h => (
                  <span key={h} style={{ fontSize: "0.5625rem", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.15em", color: S.textMuted, fontFamily: "Space Grotesk, sans-serif", textAlign: h !== "Game" ? "center" : "left" }}>{h}</span>
                ))}
              </div>
              {preds.map((r: any, i: number) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto auto auto auto", gap: 0, padding: "0.875rem 1rem", borderBottom: i < preds.length-1 ? `1px solid ${S.border}` : "none", alignItems: "center" }}
                ...
                >
                  ...
                  {["spread_result", "total_result", "moneyline_result"].map(key => (
                    <span key={key} style={{ width: "64px", textAlign: "center", padding: "0.2rem 0.375rem", background: r[key] === "HIT" ? S.greenBg : r[key] === "MISS" ? S.redBg : S.surfaceHigh, color: r[key] === "HIT" ? "#00431a" : r[key] === "MISS" ? "#ffdad6" : S.textMuted, fontSize: "0.5625rem", fontWeight: 900, textTransform: "uppercase", fontFamily: "Space Grotesk, sans-serif", margin: "0 4px" }}>
                      {r[key] ?? "—"}
                    </span>
                  ))}
          ...
          <section>
            ...
            <div style={{ background: S.surface, overflow: "hidden" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 60px 60px 60px", gap: 0, borderBottom: `1px solid ${S.border}`, padding: "0.625rem 1rem" }}>
                {["Player", "Type", "Line", "Lean", "Result"].map(h => (
                  <span key={h} style={{ fontSize: "0.5625rem", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.15em", color: S.textMuted, fontFamily: "Space Grotesk, sans-serif", textAlign: h !== "Player" ? "center" : "left" }}>{h}</span>
                ))}
              </div>
              {props.map((r: any, i: number) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 80px 60px 60px 60px", gap: 0, padding: "0.875rem 1rem", borderBottom: i < props.length-1 ? `1px solid ${S.border}` : "none", alignItems: "center" }}
                  onMouseEnter={e => (e.currentTarget.style.background = S.surfaceHigh)}
                  onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
                >
                  <div>
        ...
        {!isLoading && preds.length === 0 && props.length === 0 && (
          <p style={{ color: S.textMuted }}>No results yet. Check back after games are resolved.</p>
        )}
      </div>
    </div>
  )
