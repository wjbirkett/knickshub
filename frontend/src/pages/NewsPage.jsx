    ...
    return (<div style={{ background: S.bg, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "2rem 2.5rem", maxWidth: "1200px" }}>
        {isLoading ? <p style={{ color: S.textMuted }}>Loading...</p> : (<>
            {/* Featured 3 */}
            {featured.length > 0 && (<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "1.5rem", marginBottom: "2rem" }}>
                {featured.map((item, i) => (<a key={i} href={item.url || item.link || "#"} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "block" }}>
                    <div style={{ background: S.surface, overflow: "hidden", transition: "transform 0.2s" }} onMouseEnter={e => (e.currentTarget.style.transform = "translateY(-2px)")} onMouseLeave={e => (e.currentTarget.style.transform = "translateY(0)")}>
                      <div style={{ height: "180px", background: S.surfaceHigh, overflow: "hidden" }}>
                        {item.image && <img src={item.image} alt={item.title} style={{ width: "100%", height: "100%", objectFit: "cover", filter: "grayscale(30%)" }} onError={e => (e.target.style.display = "none")}/>}
                      </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
              {rest.map((item, i) => (<a key={i} href={item.url || item.link || "#"} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "flex", gap: "1rem", padding: "0.875rem 1rem", transition: "background 0.15s" }} onMouseEnter={e => (e.currentTarget.style.background = S.surface)} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                  <div style={{ flex: 1 }}>
