...
export default function GameHubPage() {
  ...
  const gameArticles = (articles as any[])?.filter((a: any) => a.game_date === dateStr) ?? []
  ...
  const propArticles = gameArticles.filter((a: any) => a.article_type === "prop")
  ...
  const game = (games as any[])?.find((g: any) => String(g.game_date).substring(0, 10) === dateStr)
  ...
  const fmt = (d: string) => new Date(d + "T12:00:00").toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }).toUpperCase()

  const resultBadge = (result: string | null) => {
    if (!result) return null
    return (
      <span style={{ background: result === "HIT" ? S.greenBg : S.redBg, color: result === "HIT" ? "#00431a" : "#ffdad6", padding: "0.15rem 0.5rem", fontSize: "0.5625rem", fontWeight: 900, textTransform: "uppercase", fontFamily: "Space Grotesk, sans-serif" }}>
        {result}
      </span>
    )
  }
    ...
    <div className="main-content" style={{ background: S.bg, minHeight: "100vh" }}>
      ...
      <section style={{ position: "relative", height: "420px", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: "#0a0a0a" }}>
        ...
        <div style={{ position: "relative", zIndex: 10, textAlign: "center", width: "100%", maxWidth: "900px", padding: "0 2rem" }}>
          {hasScore && (
            <span style={{ fontSize: "0.625rem", fontWeight: 700, letterSpacing: "0.3em", textTransform: "uppercase", color: S.textMuted, display: "block", marginBottom: "1rem", fontFamily: "Space Grotesk, sans-serif" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "3rem" }}>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              {!hasScore && game?.arena && (
                <span style={{ display: "block", fontSize: "0.625rem", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: S.textMuted, marginTop: "0.5rem", border: `1px solid rgba(86,67,54,0.3)`, padding: "0.25rem 0.75rem", fontFamily: "Space Grotesk, sans-serif" }}>{game.arena}</span>
              )}
            </div>
            <div style={{ textAlign: "center" }}>
          ...
          {hasScore && (
            <span style={{ display: "inline-block", marginTop: "1.5rem", fontSize: "0.625rem", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: S.textMuted, border: `1px solid rgba(86,67,54,0.3)`, padding: "0.25rem 0.75rem", fontFamily: "Space Grotesk, sans-serif" }}>
      ...
      {/* Odds Strip */}
      {(picks.spread_pick || picks.total_pick || picks.moneyline_pick) && (
        <div style={{ position: "sticky", top: 0, zIndex: 40, background: S.surface, borderBottom: `1px solid ${S.border}`, padding: "0.875rem 2.5rem", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: "3rem" }}>
          {[
          ...
          ].filter(([, v]) => v).map(([label, value, result]) => (
            <div key={label as string} style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
      ...
      <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "2.5rem", display: "grid", gridTemplateColumns: "1fr 340px", gap: "2.5rem", alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "2.5rem" }}>
            ...
            <section>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "1px", background: S.border, border: `1px solid ${S.border}` }}>
                {propArticles.map((a: any) => {
                  const propResult = gamePropResults.find((r: any) => r.slug === a.slug)
                  ...
                  return (
                    <Link key={a.slug} to={`/predictions/${a.slug}`} style={{ textDecoration: "none" }}>
                      <div style={{ background: S.surfaceHigh, padding: "1.25rem", display: "flex", justifyContent: "space-between", alignItems: "center", transition: "background 0.15s" }}
                      ...
                      >
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.375rem" }}>
                          {picks?.lean && (
                            <span style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 900, fontSize: "1rem", color: picks.lean === "OVER" ? S.green : S.red }}>{picks.lean}</span>
                          )}
          ...
          {gameArticles.length === 0 && (
            <div style={{ background: S.surface, padding: "3rem", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem", position: "sticky", top: "4rem" }}>
          ...
          {/* Quick Picks */}
          {(picks.spread_pick || picks.moneyline_pick) && (
            <div style={{ background: S.surface, padding: "1.5rem", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {picks.spread_pick && (
                  <div style={{ background: S.surfaceHigh, padding: "0.875rem 1rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          {/* Editor's Best Bet */}
          {bestBet && (
            <div style={{ background: S.surfaceHigh, padding: "1.5rem", border: `2px solid ${S.orange}`, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ marginTop: "1.5rem" }}>
                ...
                <Link to={`/predictions/${bestBet.slug}`} style={{ display: "block", width: "100%", padding: "0.75rem", background: "rgba(245,132,38,0.1)", border: `1px solid rgba(245,132,38,0.3)`, color: S.peach, fontFamily: "Space Grotesk, sans-serif", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.75rem", textAlign: "center", textDecoration: "none", transition: "all 0.15s", boxSizing: "border-box" }}
                  onMouseEnter={e => { e.currentTarget.style.background = S.orange; e.currentTarget.style.color = "#5c2b00" }}
                  onMouseLeave={e => { e.currentTarget.style.background = "rgba(245,132,38,0.1)"; e.currentTarget.style.color = S.peach }}
                >Tail This Pick</Link>
              </div>
            </div>
          )}
          ...
          <div style={{ background: S.surface }}>
            {[
              ...
              <Link key={to} to={to} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1rem 1.25rem", borderBottom: `1px solid ${S.border}`, textDecoration: "none", transition: "background 0.15s" }}
