...
export default function PlayerArchivePage() {
    ...
    const propArticles = articles?.filter((a) => a.article_type === "prop" && a.player === playerName).sort((a, b) => new Date(b.game_date).getTime() - new Date(a.game_date).getTime()) ?? [];
    ...
    const propTypes = Array.from(new Set(propArticles.map((a) => a.prop_type).filter(Boolean)));
    ...
    const getResult = (slug) => propResults.find(r => r.slug === slug);
    if (!playerName)
        return (<div className="main-content" style={{ background: S.bg, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
    ...
    return (<div className="main-content" style={{ background: S.bg, minHeight: "100vh" }}>
      ...
      <div style={{ background: S.surface, borderBottom: `1px solid ${S.border}`, padding: "2rem 2.5rem", display: "flex", alignItems: "center", gap: "2rem", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap" }}>
          ...
          {pct !== null && (<div style={{ textAlign: "center" }}>
      ...
      <div style={{ padding: "2rem 2.5rem", maxWidth: "900px" }}>
        ...
        {/* Prop type breakdown */}
        {propBreakdown.length > 0 && (<div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", marginBottom: "2rem" }}>
            {propBreakdown.map(({ pt, hits: h, total: t }) => (<div key={pt} style={{ background: S.surface, border: `1px solid ${S.border}`, padding: "0.75rem 1rem", display: "flex", flexDirection: "column", gap: "0.25rem" }}>
        ...
        {/* Articles list */}
        {isLoading ? (<p style={{ color: S.textMuted }}>Loading...</p>) : propArticles.length === 0 ? (<p style={{ color: S.textMuted, fontFamily: "Space Grotesk, sans-serif" }}>No prop articles yet for {playerName}.</p>) : (<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {propArticles.map((a) => {
                ...
                return (<Link key={a.slug} to={`/predictions/${a.slug}`} style={{ textDecoration: "none" }}>
                  <div style={{ background: S.surface, padding: "1rem 1.25rem", display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap", transition: "background 0.15s" }} onMouseEnter={e => (e.currentTarget.style.background = S.surfaceHigh)} onMouseLeave={e => (e.currentTarget.style.background = S.surface)}>
                    ...
                    <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", flexShrink: 0 }}>
                      {line && (<div style={{ textAlign: "center" }}>
                      ...
                      {result ? (<span style={{ background: result.result === "HIT" ? S.greenBg : S.redBg, color: result.result === "HIT" ? "#00431a" : "#ffdad6", padding: "0.25rem 0.625rem", fontSize: "0.625rem", fontWeight: 900, textTransform: "uppercase", fontFamily: "Space Grotesk, sans-serif", minWidth: "48px", textAlign: "center" }}>
                          {result.result === "HIT" ? "✓ HIT" : "✗ MISS"}
                        </span>) : (<span style={{ background: S.surfaceHigh, color: S.textMuted, padding: "0.25rem 0.625rem", fontSize: "0.625rem", fontWeight: 700, textTransform: "uppercase", fontFamily: "Space Grotesk, sans-serif", minWidth: "48px", textAlign: "center" }}>PENDING</span>)}
                    </div>
                  </div>
                </Link>);
            })}
