...
export default function StatsPage() {
    ...
    return (<div style={{ maxWidth: "900px" }}>
      ...
      {data && data.length === 0 && (<p style={{ color: "#6b7280" }}>Stats unavailable — stats.nba.com may be rate limiting. Try again in a minute.</p>)}

      {sorted.length > 0 && (<div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: "0.75rem", overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
            <thead>
              <tr style={{ background: "#0d0d0d" }}>
                ...
                {COLS.map(c => (<th key={c.key} onClick={() => setSort(c.key)} style={{
            ...
            <tbody>
              {sorted.map((p, i) => (<tr key={p.player_id} style={{
