// src/pages/predictions/MatchupPage.tsx
...
export default function MatchupPage() {
  ...
  const related = (allArticles ?? []).filter((a: any) => a.game_date === article.game_date && a.slug !== matchup)
    ...
    <div style={{ maxWidth: "780px", margin: "0 auto" }}>
      ...
      <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: "0.75rem", padding: "2rem", marginBottom: "1.5rem" }}>
        {article.content.split("\n").map((line: string, i: number) => {
          if (line.startsWith("## ")) return <h2 key={i} style={{ color: "#F58426" }}>{line.replace("## ", "")}</h2>
          if (line.startsWith("# ")) return <h1 key={i} style={{ color: "#F58426" }}>{line.replace("# ", "")}</h1>
        ...
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: "0.75rem", padding: "1.25rem", marginBottom: "1.5rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {related.map((a: any) => {
              ...
              return (
                <Link key={a.slug} to={`/predictions/${a.slug}`} style={{ display: "flex", alignItems: "center", gap: "0.75rem", textDecoration: "none" }}>
