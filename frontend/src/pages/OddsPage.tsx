...
export default function OddsPage() {
  ...
  const line = (odds as any[])?.[0]
  const nextGame = (schedule as any[])?.find((g: any) => g.status !== "Final")
  ...
  const fmt = (ml: number) => ml > 0 ? `+${ml}` : `${ml}`
  const fmtSpread = (s: number) => s > 0 ? `+${s.toFixed(1)}` : `${s.toFixed(1)}`
  ...
  return (
    <div className="main-content" style={{ background: S.bg, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "2rem 2.5rem", maxWidth: "800px" }}>
        {isLoading ? (
          <p style={{ color: S.textMuted, fontFamily: "Space Grotesk, sans-serif" }}>Loading odds...</p>
        ) : !line ? (
          <div style={{ background: S.surface, border: `1px solid ${S.border}`, padding: "3rem", textAlign: "center" }}>
          ...
          <>
            ...
            {/* Opponent ML */}
            {oppMl != null && (
              <div style={{ background: S.surface, border: `1px solid ${S.border}`, padding: "1rem 1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
