...
export default function TweetsPage() {
    ...
    const avgKnicksScore = finished.length
        ? (finished.reduce((a, g) => a + knicksScore(g), 0) / finished.length).toFixed(1)
        : "N/A";
    const avgOppScore = finished.length
        ? (finished.reduce((a, g) => a + oppScore(g), 0) / finished.length).toFixed(1)
        : "N/A";
    ...
    const statCard = (label, value, sub, highlight) => (<div key={label} style={{
            background: "#111827",
